...
<div class="notice-body">
  ...
  <ol>
    <li>Pay the Outstanding Amount (Including 2% Penalty) of <strong>Rs. ${formattedAmount} (Rupees ${pendingWords} Only)</strong> within Three (3) days from the date of receipt of this Notice; or</li>
    <li>If you dispute the Outstanding Amount or any component thereof, submit a detailed written representation along with complete documentary evidence within the aforesaid period on <strong><a href="mailto:${clientEmail || 'Legal@actoloan.com'}" style="color: blue; text-decoration: underline;">${clientEmail || 'Legal@actoloan.com'}</a></strong>. For an amicable solution you can also reach out to <strong>${repName}</strong> on <strong>${repPhone}</strong>.</li>
  </ol>
