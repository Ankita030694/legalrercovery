...
async function run() {
  ...
  for (const colName of collections) {
    ...
    if (doc) {
      ...
      console.log("Sample Keys & Value Types:", JSON.stringify(schema, null, 2));
      console.log("Sample Document JSON:", JSON.stringify(doc, (key, value) => {
        if (key === 'password' || key === 'otp' || key === 'loginOtp' || key === 'passwordHash') {
          return '[REDACTED_SECURITY]';
        }
        return value;
      }, 2));
    } else {
      console.log("No documents found (collection empty).");
    }
  }
